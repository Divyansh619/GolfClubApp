...
const Dashboard = () => {
  ...
  const [activeClick, setactiveClick] = useState(route.name);
...
const FetchOutStandingBills = (memberID) => {
  ...
  return (
    <View style={styles.container}>
        ...
        >
          {scrollData.map((item, index) => (
            <TouchableOpacity
              ...
              style={[
                styles.tab,
                activeClick === item.name ? styles.activeTab : null,
            ...
            >
              <Text
                style={[
                  styles.tabText,
                  activeClick === item.name ? styles.activeTabText : null,
                ]}
              >
                {item.name}
              </Text>
      ...
      {activeClick === "Dashboard" ? (
        <>{Object.keys(data).length>0&& <HomeDashboard data={data} />}</>
      ) : activeClick === "Golf booking" ? (
        <GolfBooking />
      ) : activeClick === "Bills" ? (
        <BillandTransaction FetchOutStandingBills={FetchOutStandingBills} paidBills={paidBills} transactions={transactions} outStandingData={outStandingData} />
      ) : activeClick === "Account Recharge" ? (
        <StayTuned title={"Acount Recharge"} />
      ) : activeClick === "Service Request" ? (
        <ContactUs />
      ) : (
        ""
      )}
